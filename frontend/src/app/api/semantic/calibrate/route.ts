...
export async function POST(request: Request) {
  try {
    ...
    const data = await gatewayAdminFetch(`/admin/semantic/anchors/calibrate?tenant_id=${encodeURIComponent(tenantId)}`, {
      method: 'POST',
    ...
    console.error('Semantic calibration error:', error)
    if (error instanceof GatewayAdminError) {
      return NextResponse.json({ error: error.message, details: error.details }, {
        status: error.statusCode || 500,
      })
    }
    return NextResponse.json({ error: 'Failed to calibrate semantic threshold' }, { status: 500 })
  }
}
