...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await gatewayAdminFetch(`/admin/jwt-subs/requests?${qs.toString()}`, {
      requestAuthToken: auth.token,
    })

    return NextResponse.json({ data })
  } catch (error) {
    console.error('JWT subs requests error:', error)
    ...
    return NextResponse.json(
      { error: 'Failed to fetch JWT subs raw usage requests' },
      { status: 500 }
    )
  }
}
