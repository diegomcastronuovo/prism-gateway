...
export async function POST(request: Request) {
  try {
    ...
    const data = await gatewayAdminFetch(`/admin/semantic/test?tenant_id=${encodeURIComponent(tenantId)}`, {
      method: 'POST',
    ...
    return NextResponse.json({ error: 'Failed to run semantic test' }, { status: 500 })
  }
}
