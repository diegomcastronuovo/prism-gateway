import { NextResponse } from 'next/server'
import { gatewayAdminFetch, GatewayAdminError } from '@/lib/server/gateway-admin-client'
import { requireAdminBearer } from '@/lib/server/require-admin-bearer'

export async function GET(request: Request) {
  try {
    const auth = await requireAdminBearer(request)
    if ('response' in auth) return auth.response
    const data = await gatewayAdminFetch('/admin/observability/semantic-correlation', {
      requestAuthToken: auth.token,
    })

    return NextResponse.json(data)
  } catch (error) {
    console.error('Semantic correlation error:', error)

    if (error instanceof GatewayAdminError) {
      return NextResponse.json(
        { error: error.message, details: error.details },
        { status: error.statusCode || 500 }
      )
    }

    return NextResponse.json(
      { error: 'Failed to fetch semantic correlation' },
      { status: 500 }
    )
  }
}
